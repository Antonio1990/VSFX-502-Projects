...
class ShadingRate_Rif : public RifPlugin {
	public:
			 ShadingRate_Rif(RtFloat scalingFactor);
	virtual ~ShadingRate_Rif() { }	
...
RifPlugin *RifPluginManufacture(int argc, char **argv) {
	...
	if(argc > 0)
		n = atof(argv[0]);
	n = (n < 0) ? 1 : n;
    return new ShadingRate_Rif(n);
	}
	
// Constructor
ShadingRate_Rif::ShadingRate_Rif(RtFloat scaleFactor) {
	m_filter.ShadingRate = shadingRate;  // install our custom callback
	m_rate_scaler = scaleFactor;
	}
// 
RtVoid ShadingRate_Rif::shadingRate(RtFloat rate) {
	ShadingRate_Rif *self = static_cast<ShadingRate_Rif*> (RifGetCurrentPlugin());
